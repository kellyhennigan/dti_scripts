...
if notDefined('space')
    space = 'ns'; % spatially normalized space
end
...
switch lower(space)
    ...
    case 'ns'
        ...
        coords = cellfun(@(x) dlmread(fullfile(p.data, 'CoMs',[x fdStr '_ns_CoM'])), target,'UniformOutput',0);
...
%             fdPaths = cellfun(@(x) fullfile(p.data,x,'fg_densities',method,[target{t} fdStr '.nii.gz']), subjects,'UniformOutput',0);
%         
