...
cols{1} = [244 101 7]./255; cols{2} = [ 44 129 162]./255; cols{3} = [242 173 10]./255;
   cols{4} = [.5 .5 .5];
...
%     gm = gmdistribution.fit(endpts,numClusters,'Options',options);
 gm = fitgmdist(endpts,numClusters,'Options',options);
 ...
 %% make another cluster for high uncertainty fibers?
    ...
    if uncCluster == 1
        ...
        cIdx(uncertainIdx) = numClusters + 1;
    end
    ...
    cols = cols(1:nC+uncCluster);
    
    
    h = AFQ_RenderFibers(fgCl{1},'tubes',0,'color',cols{1},'newfig',1)
   hold on 
   h2 = cellfun(@(x,y) AFQ_RenderFibers(x,'tubes',0,'color',y,'newfig',0), fgCl(2:end),cols(2:end))
